...
export interface IssueGunInput {
  personId: string;
  gunId: string;
  gunBulletsIssued?: number;
  pistolId?: string;
  pistolBulletsIssued?: number;
  weapons?: Array<{
    gunId: string;
    bulletsIssued: number;
    role?: string;
  }>;
  issueDate: string;
  issueTime: string;
...
export interface ReturnGunInput {
  actualReturnDate: string;
  actualReturnTime: string;
  gunCondition: 'Good' | 'Damaged' | 'Needs Cleaning';
  bulletsIssued?: number;
  bulletsUsed: number;
  bulletsReturned: number;
  gunBulletsIssued?: number;
  gunBulletsUsed?: number;
  gunBulletsReturned?: number;
  pistolBulletsIssued?: number;
  pistolBulletsUsed?: number;
  pistolBulletsReturned?: number;
  weapons?: any[];
  accessoriesIssued?: string[];
...
export class LeaseService {
   ...
   */
  static async issueGun(req: AuthenticatedRequest, input: IssueGunInput) {
    ...
    if (person.status === 'Blocked') {
      throw new AppError(
        `Person ${person.fullName} is BLOCKED from receiving firearms. Reason: ${person.blockedReason || 'Administrative hold'}`,
        400,
        'PERSON_BLOCKED'
      );
    }
    ...
    if (primaryGun.status !== 'Available') {
      throw new AppError(
        `Primary Gun '${primaryGun.model}' (${primaryGun.serialNumber}) is currently ${primaryGun.status}. Only AVAILABLE firearms can be issued.`,
        400,
        'GUN_NOT_AVAILABLE'
      );
    }
    ...
    if (input.pistolId && input.pistolId !== input.gunId) {
      ...
      if (secondaryGun.status !== 'Available') {
        throw new AppError(
          `Secondary firearm '${secondaryGun.model}' is currently ${secondaryGun.status}. Must be AVAILABLE.`,
          400,
          'SECONDARY_GUN_NOT_AVAILABLE'
        );
      }
    }
    ...
    if (primaryBullets > 0) {
      ...
      if (primaryAmmo.availableQuantity < primaryBullets) {
        throw new AppError(
          `Insufficient ammunition stock for ${primaryGun.calibre}. Available: ${primaryAmmo.availableQuantity}, Requested: ${primaryBullets}`,
          400,
          'INSUFFICIENT_AMMO_STOCK'
        );
      }
    }

    let secondaryAmmo: any = null;
    if (secondaryGun && secondaryBullets > 0) {
      ...
      if (secondaryAmmo.availableQuantity < secondaryBullets) {
        throw new AppError(
          `Insufficient secondary ammunition stock for ${secondaryGun.calibre}. Available: ${secondaryAmmo.availableQuantity}, Requested: ${secondaryBullets}`,
          400,
          'INSUFFICIENT_AMMO_STOCK'
        );
      }
   ...
   */
  static async returnGun(req: AuthenticatedRequest, leaseId: string, input: ReturnGunInput) {
    const userId = req.user?.id;
    ...
    if (bulletsUsed + bulletsReturned !== totalIssued) {
      throw new AppError(
        `Ammunition reconciliation mismatch: Bullets issued (${totalIssued}) must equal used (${bulletsUsed}) + returned (${bulletsReturned}).`,
        400,
        'AMMO_RECONCILIATION_MISMATCH'
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const primaryNewStatus =
        input.gunCondition === 'Damaged' || input.gunCondition === 'Needs Cleaning'
          ? 'Maintenance'
          : 'Available';
   ...
   */
  static async cancelLease(req: AuthenticatedRequest, leaseId: string) {
    const userId = req.user?.id;
