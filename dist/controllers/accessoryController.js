...
export class AccessoryController {
    static async listAccessories(req, res, next) {
        try {
    ...
    static async getAccessory(req, res, next) {
        try {
    ...
    static async createAccessory(req, res, next) {
        try {
            const { name, nameAr, category, categoryAr, availableQuantity, totalQuantity, condition, storageLocation, storageLocationAr, assignedPerson, notes, notesAr, } = req.body;
    ...
    static async updateAccessory(req, res, next) {
        try {
            const id = String(req.params.id);
        ...
        try {
            ...
            if (type === 'Stock Received' || type === 'Returned') {
                ...
                if (item.availableQuantity < delta) {
                    throw new AppError(`Cannot deduct ${delta} items. Available quantity is ${item.availableQuantity}.`, 400, 'INSUFFICIENT_STOCK');
                }
                newAvail -= delta;
                if (type === 'Damaged/Lost')
