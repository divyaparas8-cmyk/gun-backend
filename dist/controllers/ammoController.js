...
export class AmmoController {
    static async listAmmunition(req, res, next) {
        try {
    ...
    static async getAmmunition(req, res, next) {
        try {
    ...
    static async createAmmunition(req, res, next) {
        try {
            const { bulletType, bulletTypeAr, calibre, availableQuantity, minimumStockLevel, storageLocation, storageLocationAr, condition, notes, notesAr, } = req.body;
    ...
    static async updateAmmunition(req, res, next) {
        try {
            const id = String(req.params.id);
        ...
        try {
            ...
            if (type === 'Stock Received' || type === 'Bullets Returned') {
                ...
                if (ammo.availableQuantity < delta) {
                    throw new AppError(`Cannot deduct ${delta} rounds. Only ${ammo.availableQuantity} rounds available in stock.`, 400, 'INSUFFICIENT_STOCK');
                }
                newQuantity -= delta;
            }
