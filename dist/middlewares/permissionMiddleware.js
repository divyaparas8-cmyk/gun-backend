...
export const requirePermission = (permission) => {
    return async (req, res, next) => {
        try {
            ...
            if (!allowed) {
                throw new AppError(`Access denied. You lack the '${permission}' permission required for this action.`, 403, 'FORBIDDEN');
            }
            next();
        }
        catch (error) {
...
export const requireRoles = (roles) => {
    return (req, res, next) => {
        ...
        next(new AppError(`Forbidden. Access restricted to roles: [${roles.join(', ')}].`, 403, 'FORBIDDEN'));
    };
};
