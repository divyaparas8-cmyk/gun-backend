...
const runTests = async () => {
    ...
    try {
        ...
        assert('Issue blocked person rejected with 400', blockedIssueRes.status === 400 && blockedIssueRes.body.code === 'PERSON_BLOCKED');
        ...
        assert('Ammunition stock accurately decremented by 30 rounds', ammoAfterIssue?.availableQuantity === (ammoBefore?.availableQuantity || 0) - 30);
        // 11. Verify Accessory Stock Decremented
        const accAfterIssue = await prisma.accessory.findFirst({ where: { id: createdAccId } });
        assert('Accessory stock accurately decremented by 1', accAfterIssue?.availableQuantity === (accBefore?.availableQuantity || 0) - 1);
        ...
        assert('Ammunition stock restored by 20 returned rounds', ammoAfterReturn?.availableQuantity === (ammoAfterIssue?.availableQuantity || 0) + 20);
        // 18. Verify Accessory Restocked by 1
        const accAfterReturn = await prisma.accessory.findFirst({ where: { id: createdAccId } });
        assert('Accessory available quantity restored', accAfterReturn?.availableQuantity === (accBefore?.availableQuantity || 0));
        ...
        assert('Mismatched ammunition reconciliation rejected with 400', invalidReturnRes.status === 400 && invalidReturnRes.body.code === 'AMMO_RECONCILIATION_MISMATCH');
        // Cancel this lease
